...
using Malak.CommonEntities.Common;
...
namespace Malak.Api.Controllers.Common
{
    [Route("api/[controller]")]
    [Produces("application/json")]
    [ApiController]
    public class BaseResultHandlerController<TService> : ControllerBase
        where TService : class, IBaseBusinessService
    {
        ...
        protected async Task<IActionResult> EditItemResponseHandler<T>(Func<Task<ApiResponse<T>>> serviceMethod)
        {
            try
            {
                ...
                {
                    Succeeded = false,
                    ErrorMessages = GetModelStateErrors(ModelState).ToList()
                });
            }
            catch (Exception ex)
            {
